...
```{r}
library(readr)
whr = read.csv("D:\\A MATKUL IPB\\Semester 4\\Anreg\\Bahan data\\WHR2024.csv", sep = ",",
               col.names = c("Score", "Country's GDP per capita", "Social Support", "Healthy life expectancy", "Freedom to make life choices", "Generosity", "Perceptions of corruption", "Dystopia"))
head(whr)
```
```{r}
colSums(is.na(whr))
```
```{r}
whr_clean = data.frame(na.omit(whr))
colSums(is.na(whr_clean))
```
...
```{r}
y = whr_clean$Score
x = whr_clean$Country.s.GDP.per.capita
df_reg = data.frame(cbind(x,y))
head(df_reg)
```
```{r}
model = lm(y~x, df_reg)
...
Sehigga dapat disimpulkan bahwa pada taraf signifikansi 5%, selang kepercayaan untuk parameter $β_0$ adalah [ 2.154801 ,  3.018131 ] dan untuk parameter $β_1$ adalah [ 1.836221 ,  2.434756 ]
